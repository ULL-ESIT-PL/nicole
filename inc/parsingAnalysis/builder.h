...
public:
  ...
  createBool(const bool value, const std::shared_ptr<AST> &father) noexcept;

  [[nodiscard]] static std::shared_ptr<AST_CHAR>
  createChar(const char value, const std::shared_ptr<AST> &father) noexcept;

  [[nodiscard]] static std::shared_ptr<AST_DOUBLE>
  createDouble(const double value, const std::shared_ptr<AST> &father) noexcept;

  [[nodiscard]] static std::shared_ptr<AST_FLOAT>
  createFloat(const float value, const std::shared_ptr<AST> &father) noexcept;

  [[nodiscard]] static std::shared_ptr<AST_INT>
  createInt(const int value, const std::shared_ptr<AST> &father) noexcept;

  [[nodiscard]] static std::shared_ptr<AST_NULL>
  createNull(const std::shared_ptr<AST> &father) noexcept;
