...
TEST_CASE("AST_STOP within loops", "[AST_STOP]") {
  ...
  auto condition = *Builder::createCondition(
      ...
      *Builder::createBool(SourceLocation{token, token}, true));
  auto body =
      *Builder::createBody(SourceLocation{token, token},
                           std::vector<std::shared_ptr<AST_STATEMENT>>{});

  SECTION("AST_STOP in AST_FOR") {
    auto loop = *Builder::createFor(
        SourceLocation{token, token}, std::vector<std::shared_ptr<AST>>{},
        condition, std::vector<std::shared_ptr<AST>>{}, body);
    AST_STOP astStop{0, SourceLocation{token, token}, loop};
    REQUIRE(astStop.fatherLoop() == loop);
  }

  SECTION("AST_STOP in AST_WHILE") {
    auto loop =
        *Builder::createWhile(SourceLocation{token, token}, condition, body);
    AST_STOP astStop{0, SourceLocation{token, token}, loop};
    REQUIRE(astStop.fatherLoop() == loop);
  }
  ...
  SECTION("AST_STOP in AST_DO_WHILE") {
    auto loop =
        *Builder::createDoWhile(SourceLocation{token, token}, body, condition);
    AST_STOP astStop{0, SourceLocation{token, token}, loop};
    REQUIRE(astStop.fatherLoop() == loop);
  }
}
...
TEST_CASE("AST_PASS within loops", "[AST_PASS]") {
  ...
  SECTION("AST_PASS in AST_FOR") {
    auto loop = *Builder::createFor(
        SourceLocation{token, token}, std::vector<std::shared_ptr<AST>>{},
        condition, std::vector<std::shared_ptr<AST>>{}, body);
    AST_PASS astPass{0, SourceLocation{token, token}, loop};
