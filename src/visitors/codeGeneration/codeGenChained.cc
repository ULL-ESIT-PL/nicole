...
CodeGeneration::visit(const AST_CHAINED *node) const noexcept {
  ...
  switch (node->base()->type()) {
    case AST_TYPE::VAR_CALL: {
      auto addr = emitLValue(node->base().get());
      if (!addr) return createError(addr.error());
      current = *addr;
      break;
    }
    ...
    case AST_TYPE::ENUM_ACCESS: {
      auto val = emitRValue(node->base().get());
      if (!val) return createError(val.error());
      current = *val;
      break;
    }
    default:
  ...
  for (auto &op : node->operations()) {
    ...
    if (auto *attr = dynamic_cast<const AST_ATTR_ACCESS*>(op.get())) {
      // acceso a atributo → lvalue
      auto addr = emitLValue(attr);
      if (!addr) return createError(addr.error());
      next = *addr;
    }
    else if (auto *idx = dynamic_cast<const AST_INDEX*>(op.get())) {
      // acceso por índice → ¡rvalue!, devuelve el valor dentro del vector/str
      auto val = emitRValue(idx);
      if (!val) return createError(val.error());
      next = *val;
    }
    else if (auto *mcall = dynamic_cast<const AST_METHOD_CALL*>(op.get())) {
      // llamada a método → rvalue
      auto val = emitRValue(mcall);
      if (!val) return createError(val.error());
      next = *val;
    }
    else {
